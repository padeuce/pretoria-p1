...
- **Behind the scenes:** `.github/workflows/sync-instagram-feeds.yml` checks the public `@premierpadel` and `@ohpadel_club` feeds every fifteen minutes. It publishes five recent posts from each account in alternating order, stores their cover images locally, and refreshes the page carousel every fifteen minutes while visible.
